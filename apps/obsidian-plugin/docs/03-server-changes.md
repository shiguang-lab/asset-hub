...
`path` 派生的目录树无法表达**空目录**——没有文档的目录不会出现。接受此限制：目录随文档产生与消失。

因此「新建目录」入口（目录面板标题栏的按钮与右键「新建子目录」）已移除，改为在**移动到目录**弹窗里直接输入目标目录路径（如 `产品/客户项目`）。把文档放进去，目录才存在。
...
| 拖拽到目录行 | 同上 |
| 重命名目录 | 把该目录下所有文档的路径前缀换成新前缀 |
| 删除目录 | 把该目录下所有文档**上提到父目录**（顶层目录则上提到全部文档） |

批量用 `If-Match` 逐条提交，单条失败不影响其余，最后按失败数汇报。路径在服务端唯一，目标路径被占用会返回 409 `ASSET_PATH_CONFLICT`。

已知边界：回收站中的文档不参与改写（`path` 保留原值），因此恢复后会重新出现原目录。目录是「活跃文档路径的投影」，这条规则与之一致。
