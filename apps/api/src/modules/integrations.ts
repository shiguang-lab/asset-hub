...
export function registerIntegrations(app: FastifyInstance): void {
  ...
  app.post("/api/v1/integrations/git", async (req) => {
    const body = z
      ...
      .parse(req.body);
    const connection = ctx.store.createGitConnection(req.actor.workspaceId, body);
    ctx.store.audit(
      req.actor.workspaceId,
      req.actor.subject,
      "git.connect",
  ...
  app.get("/api/v1/integrations/git", async (req) =>
    ctx.store.listGitConnections(req.actor.workspaceId),
  );

  app.delete("/api/v1/integrations/git/:id", async (req, reply) => {
    const { id } = req.params as { id: string };
    const connection = ctx.store.getGitConnection(req.actor.workspaceId, id);
    if (!connection) throw notFound("Git 连接");
    ctx.store.deleteGitConnection(req.actor.workspaceId, id);
    return reply.code(204).send();
  });
  ...
  app.post("/api/v1/integrations/git/:id/sync", async (req) => {
    ...
    const connection = ctx.store.getGitConnection(req.actor.workspaceId, id);
    if (!connection) throw notFound("Git 连接");
    ctx.store.updateGitConnection(req.actor.workspaceId, id, {
      status: "syncing",
      lastError: null,
    });
    ...
    ctx.store.reserveCredits(req.actor.workspaceId, task.id, 100, `op_reserve_${task.id}`);
    ctx.bus.emit({
      eventId: nextId("evt"),
    ...
    ctx.store.audit(req.actor.workspaceId, req.actor.subject, "git.sync", id, "success", {});
  ...
  app.post("/api/v1/integrations/domains", async (req) => {
    const body = z
      ...
      .parse(req.body);
    const domain = ctx.store.createCustomDomain(req.actor.workspaceId, body.domain);
    ctx.store.audit(
      req.actor.workspaceId,
      req.actor.subject,
      "domain.add",
  ...
  app.get("/api/v1/integrations/domains", async (req) =>
    ctx.store.listCustomDomains(req.actor.workspaceId),
  );

  app.post("/api/v1/integrations/domains/:id/verify", async (req) => {
    const { id } = req.params as { id: string };
    const body = z.object({ token: z.string() }).parse(req.body);
    const domain = ctx.store.getCustomDomain(req.actor.workspaceId, id);
    if (!domain) throw notFound("域名");
    const ok = ctx.store.verifyCustomDomain(req.actor.workspaceId, id, body.token);
    if (!ok) throw badRequest("VERIFICATION_FAILED", "验证 Token 不匹配，请检查 DNS TXT 记录", {});
    ctx.store.audit(req.actor.workspaceId, req.actor.subject, "domain.verify", id, "success", {});
    return { ok: true, domain: ctx.store.getCustomDomain(req.actor.workspaceId, id) };
  });
  ...
  app.post("/api/v1/integrations/domains/:id/bind", async (req) => {
    ...
    ctx.store.bindDomainPublish(req.actor.workspaceId, id, body.publishId ?? null);
    return ctx.store.getCustomDomain(req.actor.workspaceId, id);
  });
  ...
  app.delete("/api/v1/integrations/domains/:id", async (req, reply) => {
    ...
    const domain = ctx.store.getCustomDomain(req.actor.workspaceId, id);
    if (!domain) throw notFound("域名");
    ctx.store.deleteCustomDomain(req.actor.workspaceId, id);
    return reply.code(204).send();
  });
}
