...
3. 内容密度二选一：speaker-led 使用大字号、1 个观点、1~3 条辅助信息；reading-first 使用更完整的注释和结构，但仍不得溢出，内容过多必须拆页。
4. 每页先决定焦点，再决定构图。标题不是固定占一列；可以横跨、贴边、分割、与图形互锁，但必须保持舒适行宽并避免孤字换行。
5. 图形必须表达关系：流程体现方向与阶段，矩阵体现坐标含义，趋势突出转折，架构体现层级与流向；装饰不能冒充信息图。
6. 图表必须经过设计：突出关键系列、直接标注结论、弱化无关网格、标明单位与来源；不要直接使用库的默认主题。
7. 连续页面应共享设计语言但改变视觉节奏，至少混合 hero、editorial、diagram、data-story、comparison、quote/statement、closing 等构图。
8. 页面安全区建议左右 96~140px、上下 72~100px。标题一般 58~96px，正文 26~34px；除脚注外避免低于 22px。
9. 页面必须在 16:9 容器内无滚动、无裁切、无非预期重叠。不要依赖浏览器把内容缩小到难以阅读来掩盖溢出。
`;
